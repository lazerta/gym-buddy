...
private fun requireTimestamp(value: Long, field: String) {
    require(value >= 0L) { "$field must be >= 0" }
}
...
data class SetRecord(
    ...
    init {
        ...
        requireId(executionId, "executionId")
        require(setOrdinal > 0) { "setOrdinal must be > 0" }
        requireTimestamp(startedAtUs, "startedAtUs")
    }
}
...
data class TrackingQualitySummary(
    ...
    init {
        requireId(setId, "setId")
        require(observableFrames >= 0)
        require(degradedFrames >= 0)
        require(pausedFrames >= 0)
        require(unknownFrames >= 0)
    }
}
...
data class SetSummary(
    ...
    init {
        ...
        require(assistedReps + uncertainReps <= completedReps) {
            "assistedReps + uncertainReps must not exceed completedReps"
        }
    }
}
