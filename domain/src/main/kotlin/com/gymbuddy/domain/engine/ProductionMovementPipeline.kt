...
class ProductionMovementPipeline(
    ...
    fun process(frame: PoseFrame, context: TrackingObservationContext = TrackingObservationContext()): ProductionPipelineResult {
        ...
        lastLock = lock
        val target = lock.targetCandidateIndex?.let { idx -> frame.candidates.firstOrNull { it.candidateIndex == idx } }
        val effectiveContext = if (context.observedViewClass == null && target != null) {
