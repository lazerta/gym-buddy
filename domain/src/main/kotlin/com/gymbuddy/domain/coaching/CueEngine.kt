...
class CueEngine(
    ...
    fun onInterruption(){
        history.clear()
        if(pending!=null)pendingInterrupted=true
    }
    ...
    fun evaluate(
        ...
        if(p!=null){
            ...
            val state=if(pendingInterrupted||post==null){
                CueResponseState.UNKNOWN
            }else{
                classifyResponse(p.observation,post,ruleById[p.cue.ruleId])
            }
        ...
        observations.forEach{o->
            val q=history.getOrPut(o.ruleId){ArrayDeque()}
            q.addLast(o.state)
            while(q.size>policy.persistenceWindowReps)q.removeFirst()
        }
        val eligible=observations.filter{o->
            ...
            if(q.count{it==FormObservationState.DEVIATION}<policy.requiredOccurrences){
                return@filter false
            }
            val last=lastCueAt[o.ruleId]
            if(last!=null&&rep.completedAtUs-last<policy.cooldownMs*1000L){
                return@filter false
            }
            (repeatCount[o.ruleId]?:0)<policy.maxRepeatedIdenticalCues
        }
        val selected=eligible.maxWithOrNull(
            compareBy<FormObservation>{severityRank(it.severity)}.thenBy{it.ruleId}
        )
        val cues=if(selected==null){
            emptyList()
        }else{
            val baseId="cue-"+selected.ruleId+"-"+rep.completedAtUs
            val cueId=idNamespace?.let{it+"/"+baseId}?:baseId
            val cue=CueEvent(
                cueId,selected.ruleId,rep.repId,rep.completedAtUs,selected.severity.name
            )
            pending=Pending(cue,selected)
            lastCueAt[selected.ruleId]=rep.completedAtUs
            repeatCount[selected.ruleId]=(repeatCount[selected.ruleId]?:0)+1
            listOf(cue)
        }
    ...
    private fun classifyResponse(
        ...
        if(
            rule==null||
            pre.evidenceValue==null||
            post.evidenceValue==null||
            rule.threshold==null
        )return CueResponseState.UNKNOWN
        val before=violationMagnitude(rule,pre.evidenceValue)
        val after=violationMagnitude(rule,post.evidenceValue)
        val eps=1e-9
        return when{
            after<before-eps->CueResponseState.IMPROVED
            after>before+eps->CueResponseState.WORSENED
            else->CueResponseState.UNCHANGED
        }
    }

    private fun violationMagnitude(rule:FormRule,value:Double):Double=
        when(rule.comparison){
            FormComparison.MIN_VALUE->max(0.0,rule.threshold!!-value)
            else->max(0.0,value-rule.threshold!!)
        }

    private fun severityRank(s:FormRuleSeverity)=when(s){
        FormRuleSeverity.INFO->0
        FormRuleSeverity.MINOR->1
        FormRuleSeverity.MAJOR->2
    }
}
