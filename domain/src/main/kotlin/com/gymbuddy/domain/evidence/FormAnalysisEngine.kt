...
class FormAnalysisEngine {
    fun analyze(rep:RepEvidence,rules:FormRuleSet):List<FormObservation> = rules.rules.map{rule->
        if(rule.comparison==FormComparison.MAX_ABS_DIFFERENCE){
            ...
            if(known.confidence==null||known.confidence<rule.minConfidence||rule.threshold==null)return@map unknown(rep,rule,known.confidence)
            val violation=known.value>rule.threshold
            return@map FormObservation("obs-${rep.repId}-${rule.ruleId}",rep.repId,rule.ruleId,rule.ruleVersion,if(violation)FormObservationState.DEVIATION else FormObservationState.OK,rule.severity,known.confidence,known.value)
        }
        ...
        val violation=when(rule.comparison){FormComparison.MIN_VALUE->observed<rule.threshold;else->observed>rule.threshold}
