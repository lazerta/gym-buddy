...
class CameraGuidanceEngine(
    ...
    fun evaluate(
        ...
        if (lock.state != PrimarySubjectLockState.LOCKED) return notReady(CameraGuidanceAction.CANNOT_ASSESS)
        val targetIndex = lock.targetCandidateIndex ?: return notReady(CameraGuidanceAction.CANNOT_ASSESS)
        val target = frame.candidates.firstOrNull { it.candidateIndex == targetIndex }
            ?: return notReady(CameraGuidanceAction.CANNOT_ASSESS)

        // Unknown view is not evidence of a valid view.
        val observedView = context.observedViewClass
            ?: return notReady(CameraGuidanceAction.ADJUST_ANGLE)
        ...
        if (observations.size != required.size) return notReady(CameraGuidanceAction.CANNOT_ASSESS)
        ...
        if (points.size < 2) return notReady(CameraGuidanceAction.CANNOT_ASSESS)
        val fill = maxOf(
            points.maxOf { it.x } - points.minOf { it.x },
            points.maxOf { it.y } - points.minOf { it.y },
        )
        if (fill < profile.frameFillRange.min) return notReady(CameraGuidanceAction.MOVE_CLOSER)
        if (fill > profile.frameFillRange.max) return notReady(CameraGuidanceAction.MOVE_FARTHER)
    ...
    private fun notReady(action: CameraGuidanceAction): CameraGuidanceAction {
        consecutiveReadyFrames = 0
        return action
    }
}