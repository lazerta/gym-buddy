...
@Config(sdk=[35])
class PopulatedStep3RepairMigrationTest {
    @Test fun genuineVersion8HistoryRetainsUnknownSemanticsDuringMigrationAndReopen() {
        val context=ApplicationProvider.getApplicationContext<Context>()
        val name="populated-v8-${System.nanoTime()}.db"
            ...
            SupportSQLiteOpenHelper.Configuration.builder(context).name(name)
                .callback(object:SupportSQLiteOpenHelper.Callback(8){
                    override fun onCreate(db:SupportSQLiteDatabase){
                        ...
                        GymBuddyMigrations.MIGRATION_7_8.migrate(db)
                        db.execSQL("INSERT INTO workout_sessions VALUES ('session',100,1000)")
                        db.execSQL("INSERT INTO exercise_executions (executionId,sessionId,exerciseId,startedAtUs,startedAtEpochMs) VALUES ('exec','session','dumbbell_lateral_raise',200,1100)")
                        db.execSQL("INSERT INTO sets (setId,executionId,setOrdinal,startedAtUs,actualLoadValue,actualLoadUnit,startedAtEpochMs) VALUES ('set','exec',1,300,25.0,'lb',1200)")
                        ...
                        db.execSQL("INSERT INTO rep_evidence (repId,setId,repOrdinal,stepId,primitive,startedAtUs,completedAtUs,classification) VALUES ('rep','set',1,'cycle','RAISE',350,900,'NORMAL')")
                        ...
                        db.execSQL("INSERT INTO workout_flow_states (checkpointId,completedSetId,focus,plannedNextLoadValue,plannedNextLoadUnit,restStartedAtEpochMs,state) VALUES ('active','set','Legacy focus',30.0,'lb',2000,'REST')")
        ...
        try {
            helper.writableDatabase.query("PRAGMA table_info(sets)").use{c->
                ...
                assertFalse("Historical input cannot already contain v9 columns",columns.contains("actualResistanceKind"))
            }
            helper.close()
            repeat(2){
                ...
                try {
                    ...
                    assertEquals(ResistanceKind.UNKNOWN,set.set.actualLoad!!.resistanceKind)
                    assertEquals(LoadMeasurementMode.UNKNOWN,set.set.actualLoad!!.measurementMode)
                    ...
                    assertNull(rest.clockAnchor)
                    assertEquals(ResistanceKind.UNKNOWN,rest.plannedNextLoad!!.resistanceKind)
                    assertEquals(LoadMeasurementMode.UNKNOWN,rest.plannedNextLoad!!.measurementMode)
